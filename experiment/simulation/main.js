...
const VOLTAGE_STEP = 1.0;     // 1 kV per click â†’ breakdown at ~27 clicks for 10 mm
...
function recalcBreakdown() {
  ...
  // Air dielectric strength â‰ˆ 3 kV/mm (Paschen's law)
  const base = 3.0 * state.gap * eta * oilFactor;
  // Â±8 % scatter per test run
...
function updateSettingsInfo() {
  ...
  const eta = SHAPE_ETA[state.shape];
  el.textContent =
    "Shape Factor Î· = " + eta.toFixed(2) +
    " Â· Gap = " + state.gap + " mm" +
    " Â· Oil Quality = " + Math.round(oilFactor * 100) + "%" +
    " Â· Expected V_bd â‰ˆ " + breakdownVoltage.toFixed(1) + " kV";
}
...
function triggerBreakdown() {
  ...
  msgBox.textContent =
    "âš¡ BREAKDOWN at " + record.kv + " kV! (" +
    SHAPE_LABELS[record.shape] + " Â· " + record.gap + " mm)";
...
function changeOil() {
  ...
  resetTest();
  const msgBox = document.getElementById("messageBox");
  msgBox.textContent = "Oil changed â€” all readings reset.";
...
function drawGraph() {
  ...
  state.graphPoints.forEach(function (p) {
    if (p.breakdown) {
      ...
      ctx.textAlign = "center";
      ctx.fillText("âš¡", toX(p.step), toY(p.v) - 10);
      ctx.restore();
    }
  });
